...
class InputManager
{
     ...
     *
     * Params:
     *   eventManager = The event manager to use for input state.
     */
            ...
            else
            {
                ...
                switch(type)
                {
                    ...
                    case BindingType.GamepadAxis:
                        result = cast(int)SDL_GetGamepadAxisFromString(svalue.ptr);
                        break;
                    case BindingType.GamepadButton:
                        result = cast(int)SDL_GetGamepadButtonFromString(svalue.ptr);
                        break;
                    default:
                        break;
                }
    ...
    bool getButton(Binding binding)
    {
        switch(binding.type)
        {
            ...
            case BindingType.GamepadButton:
                if (binding.deviceIndex < MAX_GAME_INPUT_DEVICES)
                    if (eventManager.gamepadButtonPressed[binding.deviceIndex][binding.button])
                        return true;
                break;
    ...
    bool getButtonUp(string name)
    {
        ...
        for(int i = 0; i < b.length; i++)
        {
            ...
            switch(binding.type)
            {
                ...
                case BindingType.GamepadButton:
                    if (binding.deviceIndex < MAX_GAME_INPUT_DEVICES)
                        if (eventManager.gamepadButtonUp[binding.deviceIndex][binding.button])
                            return true;
                    break;
    ...
    bool getButtonDown(string name)
    {
        ...
        for(int i = 0; i < b.length; i++)
        {
            ...
            switch(binding.type)
            {
                ...
                case BindingType.GamepadButton:
                    if (binding.deviceIndex < MAX_GAME_INPUT_DEVICES)
                        if (eventManager.gamepadButtonDown[binding.deviceIndex][binding.button])
                            return true;
                    break;
    ...
    float getAxis(string name)
    {
        ...
        for(int i = 0; i < b.length; i++)
        {
            ...
            switch(binding.type)
            {
                ...
                case BindingType.GamepadAxis:
                    if (eventManager.gamepadAvailable)
                    {
                        if (binding.deviceIndex < MAX_GAME_INPUT_DEVICES)
                            value = eventManager.gamepadAxis(binding.deviceIndex, binding.axis);
                    }
                    break;
