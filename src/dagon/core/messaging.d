...
class MessageBroker: Owner
{
    ...
    void update()
    {
        ...
        {
            Event event = eventManager.inboxEventQueue[i];
            ...
            {
                eventBuffer[numCollected] = eventManager.inboxEventQueue[i];
                numCollected++;
                
                // Cancel the original message/task
                if (event.type == EventType.Message || event.type == EventType.Task)
                    eventManager.inboxEventQueue[i].type = EventType.Cancelled;
            }
            else
            {
                // TODO
