...
- `MouseButtonDown`, `MouseButtonUp` - mouse button pressed/released. Reports mouse button codes (via `Event.button`) defined as `MB_*` constants in `dagon.core.scancodes`. Standard buttons are `MB_LEFT` (1), `MB_MIDDLE` (2), `MB_RIGHT` (3). Some mice also have additional side-mounted buttons `MB_X1` (4) and `MB_X2` (5)
...
### Gamepad
- `GamepadAdd` - gamepad plugged in. Reports `Event.deviceIndex` and `Event.deviceType`
- `GamepadRemove` - gamepad unplugged. Reports `Event.deviceIndex`
- `GamepadButtonDown`, `GamepadButtonUp` - gamepad button pressed/released. Reports `Event.deviceIndex` and a button type (via `Event.gamepadButton`) defined as `GB_*` constants in `dagon.core.scancodes`
- `GamepadAxisMotion` - gamepad axis moved. Reports `Event.deviceIndex`, an axis type (via `Event.gamepadAxis`) defined as `GA_*` constants in `dagon.core.scancodes`, and an axis value (via `Event.gamepadAxisValue`) as floating-point in -1..1 range. SDL reports axis values as integers, and Dagon normalizes them, so that these values can be used properly in further calculations. You can fine-tune normalization by changing `EventManager.gamepadAxisThreshold` (32639 by default), though this is usually not needed for most devices
...
- `Minimize`, `Restore` - window is minimized or restored. Reports no data
...
Dagon, like SDL, provides two abstraction layers to work with game input devices - joysticks and gamepads. The distinction between them lies in the level of standardization.
...
Gamepad is a joystick that SDL recognizes as a "standard gamepad" (XInput, DualShock, Switch Pro, etc.). Gamepads have fixed standard buttons and axes: A/B/X/Y, left and right triggers/shoulders, sticks, d-pad. SDL allows the application to access buttons by action type, rather than by number. Gamepad API is very convenient for adapting the input logic to different devices—the code is the same for all gamepads that SDL recognizes.
