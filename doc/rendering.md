...
IBL is a standard technique to achieve approximated global illumination in a very computationally inexpensive way, which works best in outdoor scenes. The "infinitely far" environment of a scene (such as the sky and the surrounding landscape) is pre-baked into an environment map. Environment maps are typically captured using 360° photography, or rendered from 3D scenes.

Dagon implements an industry-standard IBL method known as the split sum approximation [Karis 2013]. It splits the rendering equation into two precomputable parts: a pre-filtered environment map and a 2D lookup table, sometimes called a BRDF integration map. This red-green LUT outputs a scale (red channel) and an offset (green channel) for the Fresnel term for different roughness values and viewing angles.

To provide ambient diffuse lighting, the engine computes a diffuse irradiance cube map, which solves the Lambertian diffuse integral over the hemisphere. This is achieved via cosine-weighted importance sampling.

The specular pre-filtered cube map is generated by convolving the input cube map using GGX BRDF for different roughness levels. They are stored in the mip chain of the cube map and then sampled in the shader to get a specular reflection for the given material roughness. To eliminate noise from punctual light sources during specular convolution, Dagon implements importance sampling. This technique calculates the probability density function (PDF) of each generated light vector relative to the half-vector space, mapping the sample's solid angle onto the texture's solid angle to dynamically determine the appropriate mip-level for filtering.

All cube maps can be pre-baked using an external tool or generated in the engine at run time from an equirectangular map. Dagon implements this via GPU-accelerated importance sampling using the van der Corput-Hammersley distribution on a hemisphere [Dammertz 2012]. The computation is highly optimized and exceptionally fast.
...
Screen-space reflections are a technique for achieving approximated dynamic reflections in real time. The technique operates per pixel by raymarching through the depth buffer along the reflection direction and sampling the HDR buffer when the ray intersects reconstructed geometry. Given sufficient depth and screen-space precision, the basic technique can produce mirror-like reflections on smooth surfaces.

To support physically-based reflections on rough surfaces, Dagon uses stochastic sampling: the reflection ray is generated in a randomized direction, effectively sampling the specular lobe of the surfaces's BRDF. The microfacet half-vectors are importance-sampled via the GGX Visible Normal Distribution Function (VNDF) described in *"Sampling Visible GGX Normals with Spherical Caps"* [J. Dupuy, A. Benyoub 2023]. To provide high-quality pseudo-random samples, the engine uses a GPU-optimized Permuted Congruential Generator (PCG) [O'Neill 2014] to generate per-pixel stochastic jitter. This avoids the visible lattice artifacts and structured patterns that can occur with lower-quality hash functions.

Stochastic sampling inherently introduces high-frequency Monte Carlo noise due to the low number of samples available per pixel. Dagon resolves this noise through temporal accumulation. Reflection data is reprojected between frames using motion vectors and accumulated with an exponential moving average, effectively increasing the number of samples over time. This produces stable and visually convincing glossy reflections at a low runtime cost.

Like all screen-space techniques, SSR suffers from inherent information discontinuities. Ray traversal is limited to geometry represented in the current frame's depth buffer, so rays that leave the visible screen or pass behind missing screen-space information cannot produce valid intersections. This results in holes and gaps in reflections and causes reflections to disappear when their source geometry moves outside the screen—for example, when the camera looks downward. Consequently, SSR is most practical when combined with a global reflection fallback, typically provided by environment probes or image-based lighting.
...
- Eric Heitz, [Sampling the GGX Distribution of Visible Normals](https://jcgt.org/published/0007/04/01/). Journal of Computer Graphics Techniques, 2018
- Jonathan Dupuy, Anis Benyoub (Intel Corporation), [Sampling Visible GGX Normals with Spherical Caps](https://arxiv.org/abs/2306.05044), High-Performance Graphics 2023
