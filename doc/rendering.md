...
To support reflections on rough surfaces, Dagon uses stochastic sampling: the reflection ray is generated in a randomized direction, effectively sampling the specular lobe of the material's BRDF. The reflection direction is generated using the GGX Visible Normal Distribution Function (VNDF) described by Eric Heitz [2018]. To provide high-quality pseudo-random samples, the engine uses a GPU-optimized Permuted Congruential Generator (PCG) [O'Neill 2014] to generate per-pixel stochastic jitter. This avoids the visible lattice artifacts and structured patterns that can occur with lower-quality hash functions and pseudo-random generators.
