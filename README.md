...
- All Entities are static by default, and their model matrices are not recalculated each frame to reduce CPU overhead. For dynamic updates enable `Entity.autoUpdate` or use custom `EntityController` (partly analogous to old `EntityComponent`)
